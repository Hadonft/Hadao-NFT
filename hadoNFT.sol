...
contract HADODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HADO_Dividend_Tracker", "HADO_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HADO_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "HADO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HADO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HADO_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HADO_Dividend_Tracker: Cannot update claimWait to same value");
...
contract HADO is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    HADODividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 105000 * (10**9);
    uint256 public maxBuyTransactionAmount = 210000 * (10**9);
    uint256 public swapTokensAtAmount = 210000 * (10**9);
    ...
    constructor() ERC20("HADO", "HDO") {
        BNBRewardsFee = 4;
        liquidityFee = 5;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x9a7e85f0380CD219E70aE7E6777ac65A4abF0D9a);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new HADODividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HADO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HADO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HADO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HADO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HADO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HADO: Cannot update gasForProcessing to same value");
